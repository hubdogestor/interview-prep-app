  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[90vw] sm:max-h-[90vh] p-0 bg-black text-white border-none">
        {open ? <TeleprompterContent title={title} content={content} /> : null}
      </DialogContent>
    </Dialog>
  );
}
...
function TeleprompterContent({ title, content }: TeleprompterContentProps) {
      ...
      <DialogHeader className="px-6 py-4 border-b border-white/10 flex-shrink-0">
        ...
        {showSettings && (
          <div className="mt-4 space-y-4 p-4 bg-white/5 rounded-lg">
            ...
            <TextStats text={content} className="text-white/50 justify-center" />
          </div>
        )}
      </DialogHeader>
